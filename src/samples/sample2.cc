...
static CFComponentLib theLib("TESTCOMP2", create_me, set_me_up, destroy_me);
...
create_me(const char* inst_name)
{
    ...
    return new TestComp2(inst_name);
}
...
{ 
    TestComp2* t = (TestComp2*) comp;

    cfGetRegistry()->registerIface(comp, (ITest2*)t);
...
int 
TestComp2::connect(CFComponent* other, char* iface, int argc, char** argv)
{
...
int
TestComp2::disconnect(CFComponent* other, char* iface)
{
    (void) other;  
    (void) iface;
    return 1;
...
int
TestComp2::set(char* varName, char* varValue)
{
